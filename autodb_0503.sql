...
CREATE TABLE IF NOT EXISTS `autoDB`.`BODYTYPE` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `description_UNIQUE` (`btype` ASC, `description` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `autoDB`.`TRIM` (
  ...
  PRIMARY KEY (`ID`, `BODYTYPE_ID`),
  UNIQUE INDEX `displayname_UNIQUE` (`displayname` ASC),
  INDEX `fk_TRIM_BODYTYPE1_idx` (`BODYTYPE_ID` ASC),
  CONSTRAINT `fk_TRIM_BODYTYPE1`
    FOREIGN KEY (`BODYTYPE_ID`)
    REFERENCES `autoDB`.`BODYTYPE` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `autoDB`.`ENGINE` (
  ...
  `capacity` DECIMAL(4,1) NOT NULL DEFAULT 0.1,
...
CREATE TABLE IF NOT EXISTS `autoDB`.`MODEL` (
  ...
  PRIMARY KEY (`ID`, `MAKE_ID`, `TRIM_ID`, `ENGINE_ID`),
  UNIQUE INDEX `name_UNIQUE` (`name` ASC),
  INDEX `fk_MODEL_MAKE1_idx` (`MAKE_ID` ASC),
  INDEX `fk_MODEL_TRIM1_idx` (`TRIM_ID` ASC),
  INDEX `fk_MODEL_ENGINE1_idx` (`ENGINE_ID` ASC),
  CONSTRAINT `fk_MODEL_MAKE1`
    FOREIGN KEY (`MAKE_ID`)
    REFERENCES `autoDB`.`MAKE` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_MODEL_TRIM1`
    FOREIGN KEY (`TRIM_ID`)
    REFERENCES `autoDB`.`TRIM` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_MODEL_ENGINE1`
    FOREIGN KEY (`ENGINE_ID`)
    REFERENCES `autoDB`.`ENGINE` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `autoDB`.`VEHICLE` (
  ...
  UNIQUE INDEX `VIN_UNIQUE` (`VIN` ASC, `plate` ASC),
...
-- -----------------------------------------------------
-- Table `autoDB`.`EBUILDS`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `autoDB`.`EBUILDS` (
  `ID` INT NOT NULL,
  `edescl` VARCHAR(45) NOT NULL,
  `edescs` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `edescs_UNIQUE` (`edescs` ASC))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `autoDB`.`MODELNAMES`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `autoDB`.`MODELNAMES` (
  `ID` INT NOT NULL,
  `NAME` VARCHAR(45) NOT NULL,
