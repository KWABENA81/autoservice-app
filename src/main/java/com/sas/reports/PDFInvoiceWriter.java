...
public class PDFInvoiceWriter {
	...
	private final Font roman_times_32N = new Font(FontFamily.TIMES_ROMAN, 32);
	...
	private Float labourCosts;
	private Float totalDuration;
	private Float serviceTotal;

	public PDFInvoiceWriter(ServiceOrder serviceOrder) {
		this.sOrder = serviceOrder;
		this.invoiceFileName = XMLUtils.DEF_INVOICE_PATH + "//S" + this.sOrder.getJobID() + ".pdf";
	...
	private PdfPTable createSubTable() throws DocumentException {
		...
		Font font06 = new Font(FontFamily.HELVETICA, 6);
		Font font08 = new Font(FontFamily.HELVETICA, 8);
		...
		table.addCell(createCell(sbuilder.toString(), font08, PdfPCell.BOX, 1, 1, Element.ALIGN_RIGHT));
		float ftax = getTaxes();
		float saleTax = (labourCosts + itemCosts) * ftax;
		sbuilder = new StringBuilder();
		sbuilder.append(currency.format(new Float(saleTax)));
		table.addCell(createCell(sbuilder.toString(), font08, PdfPCell.BOX, 1, 1, Element.ALIGN_RIGHT));
	...
	private float getTaxes() {
		Float ftax = 0.0f;
		Integer nValue = 0;
		nValue += XMLUtils.DEF_TAX_RATE0;
		nValue += XMLUtils.DEF_TAX_RATE1;
		nValue += XMLUtils.DEF_TAX_RATE2;
		nValue += XMLUtils.DEF_TAX_RATE3;
		ftax = new Float(nValue / 100);
		return ftax;
	}
	...
	public PdfPTable createInvoiceDetails() throws DocumentException {
		...
		for (Work work : workList) {
			...
			if (item != null) {
				if (item.getInventory().getPartNumber().equals(XMLUtils.DEF_PART_NUMBER)) {
					...
					//
					sb = new StringBuilder();
					float dltotal = item.getPrice() * item.getQuantity();
			...
			//
			sb = new StringBuilder();
			sb.append("Labour: ").append(work.getWorkDesc());
		...
		for (int row = 0; row < nRows; row++) {
			for (int col = 0; col < nCols; col++) {
				cells[row][col] = new Phrase("\u00a0");
				cell = new PdfPCell(cells[row][col]);
				table.addCell(cell);
			}
		}
		return table;
	...
	private PdfPTable createHeaderDetails() throws DocumentException {
		...
		table.addCell(createCell(sbuffer.toString(), roman_times_32N, PdfPCell.BOTTOM, 4, 2, Element.ALIGN_LEFT));
		...
		table.addCell(createCell(sbuffer.toString(), roman_times_7N, PdfPCell.NO_BORDER, 1, 1, Element.ALIGN_RIGHT));
		sbuffer = new StringBuilder();
		sbuffer.append("Phone: ").append(XMLUtils.COMPANY_PHONE);
		if (!XMLUtils.COMPANY_FAX.isEmpty()) {
			sbuffer.append("Fax: ").append(XMLUtils.COMPANY_FAX);
		}
		...
		// if (!XMLUtils.COMPANY_FAX.isEmpty()) {
		// sbuffer.append("Fax: ").append(XMLUtils.COMPANY_FAX);
		// table.addCell(createCell(sbuffer.toString(), roman_times_7N,
		// PdfPCell.BOTTOM, 1, 1, Element.ALIGN_RIGHT));
		// }
